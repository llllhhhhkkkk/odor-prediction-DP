...
from sklearn.metrics import roc_auc_score, accuracy_score, precision_score, recall_score, f1_score, \
    precision_recall_fscore_support, roc_curve, auc as sklearn_auc
...
def benchmark_ensemble_boosting(fold, train_dataset, test_dataset, n_models, nb_epoch, mean_auc = 0.0):
    # learning_rate = 0.001
    ...
    k_fold_datasets = splitter.k_fold_split(train_dataset, k=n_models, directories=dir)
    ...
    for i in tqdm(range(n_models)):
        train_data = k_fold_datasets[i][1]
        ...
        model = SCENE_MPNNPOMModel(n_tasks=n_tasks,
                                   ...
                                   optimizer_name='adam',
                                   log_frequency=32,
                                   model_dir=f'./ensemble_cv_exp/boosting_single_model/model/fold_{fold + 1}/experiments',
                                   device_name='cuda')
        if i > 0:
            model.restore(f'./ensemble_cv_exp/boosting_single_model/model/fold_{fold + 1}/experiments/checkpoint1.pt')
        ...
        path = f'./ensemble_cv_exp/boosting_single_model/result/fold_{fold + 1}'
...
plt.savefig("./ensemble_cv_exp/AUC-ROC/ensemble_boosting.png")

cv_mean_result = np.mean(folds_results)
cv_mean_micro_averages = np.mean(micro_averages, axis=0)

with open("./ensemble_cv_exp/final_score_boosting_single_model.txt", 'w+') as f:
