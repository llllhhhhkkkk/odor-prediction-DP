...
try:
    ...
    from ..layers.pom_mpnn_GMM import CustomMPNNGNN as CustomMPNNGMM
    from ..layers.pom_mpnn_DGN import CustomMPNNGNN as CustomMPNNDGN
...
class MPNNPOM(nn.Module):
    def __init__(self,
        ...
        self.mpnn_gmm: nn.Module = CustomMPNNGMM(
            node_in_feats=number_atom_features,
            node_out_feats=node_out_feats,
            edge_in_feats=number_bond_features,
            edge_hidden_feats=edge_hidden_feats,
        ...
        self.mpnn_dgn: nn.Module = CustomMPNNDGN(
            node_in_feats=number_atom_features,
            node_out_feats=node_out_feats,
            edge_in_feats=number_bond_features,
            edge_hidden_feats=edge_hidden_feats,
    ...
    def forward(
        ...
        node_encodings_gmm: torch.Tensor = self.mpnn_gmm(g, node_feats, edge_feats)
        molecular_encodings_gmm: torch.Tensor = self._readout(g, node_encodings_gmm, edge_feats)
        ...
        node_encodings_dgn: torch.Tensor = self.mpnn_dgn(g, node_feats, edge_feats)
        molecular_encodings_dgn: torch.Tensor = self._readout(g, node_encodings_dgn, edge_feats)

        if self.readout_type == 'global_sum_pooling':
            molecular_encodings = F.softmax(molecular_encodings, dim=1)
            molecular_encodings_scene = F.softmax(molecular_encodings_scene, dim=1)
            # molecular_encodings_rgcn = F.softmax(molecular_encodings_rgcn, dim=1)
            # molecular_encodings_gmm = F.softmax(molecular_encodings_gmm, dim=1)
            # molecular_encodings_schnet = F.softmax(molecular_encodings_schnet, dim=1)
            molecular_encodings_dgn = F.softmax(molecular_encodings_dgn, dim=1)


        embeddings: torch.Tensor
        out: torch.Tensor
        embeddings, out = self.ffn(torch.cat((molecular_encodings,
                                              molecular_encodings_scene,
                                              molecular_encodings_dgn), dim=1))
