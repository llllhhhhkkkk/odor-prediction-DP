...
class FeatureAttention(nn.Module):
    def __init__(self, feature_dim, num_features=3):
        ...
        self.attention = nn.Sequential(
            ...
            nn.Softmax(dim=1)  # softmax 应用在特征维度
        )

    def forward(self, features):
        # features: [batch_size, feature_dim, num_features]
        # 计算每个特征的注意力权重
        attn_weights = self.attention(features.mean(dim=2))  # 无需保持维度
        # attn_weights 形状: [batch_size, num_features]

        # 使用注意力权重对特征进行加权求和
        attn_weights = attn_weights.unsqueeze(1)  # 调整形状以匹配特征维度 [batch_size, 1, num_features]
        weighted_features = torch.matmul(features, attn_weights.transpose(1, 2))  # 在正确的维度上执行加权求和
        return weighted_features.squeeze(2)  # 移除多余的维度以匹配期望的输出形状 [128, 400]
...
class MPNNPOM(nn.Module):
    ...
    * Perform the final prediction using a feed-forward layer.
    ...
    def __init__(self,
        ...
        self.feature_attention = FeatureAttention(feature_dim=400)
    ...
    def forward(
        ...
        * When self.mode = 'classification',
        ...
        features = torch.stack((molecular_encodings,
                   molecular_encodings_scene,
                   molecular_encodings_rgcn), dim=2)

        combined_features = self.feature_attention(features)
