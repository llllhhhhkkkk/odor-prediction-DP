...
class DeepFeatureInteraction(nn.Module):
    def __init__(self, feature_dim, num_models=3):
        super().__init__()
        self.interaction_layers = nn.Sequential(
            nn.Linear(feature_dim * num_models, 512),
            nn.ReLU(),
            nn.Linear(512, feature_dim)
        )
    def forward(self, features):
        # features: [batch_size, feature_dim, num_models]
        features_flat = features.view(features.shape[0], -1)
        return self.interaction_layers(features_flat)
...
class MPNNPOM(nn.Module):
    ...
    * Perform the final prediction using a feed-forward layer.
    ...
    def __init__(self,
        ...
        self.deep_interaction = DeepFeatureInteraction(feature_dim=400)
    ...
    def forward(
        ...
        * When self.mode = 'classification',
        ...
        features = torch.cat((molecular_encodings,
                   molecular_encodings_scene,
                   molecular_encodings_rgcn), dim=1)
        combined_features = self.deep_interaction(features)
