...
from openpom.models.mpnn_pom_attention import MPNNPOMModel
...
def benchmark_ensemble(fold, train_dataset, test_dataset, n_models, nb_epoch, mean_auc):
    ...
    for i in tqdm(range(n_models)):
        model = MPNNPOMModel(n_tasks=n_tasks,
                             ...
                             optimizer_name='adam',
                             log_frequency=32,
                             model_dir=f'./ensemble_cv_exp/attention/ensemble_fold_{fold + 1}/experiments_{i + 1}',
                             device_name='cuda')
    ...
    for i in range(n_models):
        model = MPNNPOMModel(n_tasks=n_tasks,
                             ...
                             model_dir=f'./ensemble_cv_exp/attention/ensemble_fold_{fold + 1}/experiments_{i + 1}',
                             device_name='cuda')
        model.restore(f"./ensemble_cv_exp/attention/ensemble_fold_{fold + 1}/experiments_{i + 1}/checkpoint2.pt")
        # test_scores = model.evaluate(test_dataset, [metric])['roc_auc_score']
...
plt.savefig("./ensemble_cv_exp/AUC-ROC/ensemble_attention.png")

with open("./ensemble_cv_exp/final_score_attention.txt", 'w+') as f:
